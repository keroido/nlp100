...
for block in ans[n]:
    if len([b.pos for b in block.morphs if b.pos == '名詞']) > 0:
        if len([b.pos for b in ans[n][int(block.dst)].morphs if b.pos == '動詞']) > 0:
            tmp1 = ''.join([b.surface for b in block.morphs])
            tmp2 = ''.join([b.surface for b in ans[n][int(block.dst)].morphs])
